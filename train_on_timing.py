...
def compare_max_depth():
    ...
    cases.append( ('2',  X_train, y_train, X_val, y_val, {'seed':rs, 'n_estimators':60,'eta':0.1,'max-depth':2,'subsample':0.7, 'colsample_bytree':0.7, "eval_metric": "mlogloss","objective": "multi:softprob", "num_class": 12}, "xgb") )
    cases.append( ('3',  X_train, y_train, X_val, y_val, {'seed':rs, 'n_estimators':60,'eta':0.1,'max-depth':3,'subsample':0.7, 'colsample_bytree':0.7, "eval_metric": "mlogloss","objective": "multi:softprob", "num_class": 12}, 'xgb') )
    cases.append( ('4',  X_train, y_train, X_val, y_val, {'seed':rs, 'n_estimators':60,'eta':0.1,'max-depth':4,'subsample':0.7, 'colsample_bytree':0.7, "eval_metric": "mlogloss","objective": "multi:softprob", "num_class": 12}, 'xgb') )
    cases.append( ('5',  X_train, y_train, X_val, y_val, {'seed':rs, 'n_estimators':60,'eta':0.1,'max-depth':5,'subsample':0.7, 'colsample_bytree':0.7, "eval_metric": "mlogloss","objective": "multi:softprob", "num_class": 12}, 'xgb') )
    cases.append( ('6',  X_train, y_train, X_val, y_val, {'seed':rs, 'n_estimators':60,'eta':0.1,'max-depth':6,'subsample':0.7, 'colsample_bytree':0.7, "eval_metric": "mlogloss","objective": "multi:softprob", "num_class": 12}, 'xgb') )
...
if __name__ == "__main__":
    ...
    print('Read train...')
    train = pd.read_csv(inputdir + 'features.csv', dtype={'device_id': np.str})
    train = map_column(train, 'gender')
    train = map_column(train, 'group')
    train = pd.merge(train, pbd, how='left', on='device_id', left_index=True)
    train.fillna(-1, inplace=True)
    
    X = train.drop(['Unnamed: 0', 'device_id','gender','age','group'],axis=1)
    y = train['group']
    ...
    a = df.merge(train.drop(['Unnamed: 0', 'gender','age','group','phone_brand','device_model'],axis=1), how='left',on='device_id')
    a = pd.merge(a, pbd, how='left', on='device_id')
    a = map_column(a, 'group')
    a.fillna(0,inplace=True)
    
    X = a.drop(['device_id','gender','age','group'],axis=1)
    y = a['group']
    
    compare_max_depth()
    
    # Normalize events
    ne_h_1 = train.filter(regex=("h[0-9]{1,2} nevents")).mean()
    for col in ne_h_1.index:
        train[col] = train[col] / ne_h_1.loc[col]
    
    ne_d_1 = train.filter(regex=("d[0-9]{1,2} nevents")).mean()
    for col in ne_d_1.index:
        train[col] = train[col] / ne_d_1.loc[col]
