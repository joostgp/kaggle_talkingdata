...
if __name__ == "__main__":
    ...
    data_tr = pd.read_csv('./data_ori/gender_age_train.csv', dtype={'device_id': np.str})
    ...
    print('Read events...')
    events = pd.read_csv('./data_ori/events.csv')
    ...
    X = train[['counts','phone_brand','device_model','age_group','gender']]
    y = train['group']
    ...
    # Test on age/gender separately
    params = {'n_estimators':500,'eta':0.1,'max-depth':3,'subsample':0.7, 'colsample_bytree':0.7, "eval_metric": "mlogloss","objective": "multi:softprob", "num_class": 6}
    fy = 'age_group'    
    (gbm_age, eval_result) = get_xgboost_classifier(X_train[f], X_train[fy], X_val[f], X_val[fy], params, rs=2016, output_eval=True)
    
    params = {'n_estimators':500,'eta':0.1,'max-depth':3,'subsample':0.7, 'colsample_bytree':0.7, "eval_metric": "mlogloss","objective": "multi:softprob", "num_class": 2}    
    fy = 'gender'
    (gbm_gender, eval_result) = get_xgboost_classifier(X_train[f], X_train[fy], X_val[f], X_val[fy], params, rs=2016, output_eval=True)
    
    y_val_check_age = gbm_age.predict(xgb.DMatrix(X_val[f]), ntree_limit=gbm_age.best_iteration)
    score_age = log_loss(X_val['age_group'], y_val_check_age)
    
    y_val_check_gender = gbm_gender.predict(xgb.DMatrix(X_val[f]), ntree_limit=gbm_gender.best_iteration)
    score_gender = log_loss(X_val[fy], y_val_check_gender)
    
    df = combine_prob(y_val_check_age, y_val_check_gender) 
    score_comb = log_loss(y_val, df.as_matrix())
    
    # Test on group
    params = {'n_estimators':500,'eta':0.1,'max-depth':3,'subsample':0.7, 'colsample_bytree':0.7, "eval_metric": "mlogloss","objective": "multi:softprob", "num_class": 12}  
    (gbm, eval_result) = get_xgboost_classifier(X_train[f], y_train, X_val[f], y_val, params, rs=2016, output_eval=True)
    
    y_val_check = gbm.predict(xgb.DMatrix(X_val[f]), ntree_limit=gbm_age.best_iteration)
    score = log_loss(y_val, y_val_check)
    
    print "Separate score: {:.4f}".format(score_comb)
    print "Total score: {:.4f}".format(score)
    
    
    X_test = test[['counts','phone_brand','device_model']]
