...
#    print time.time()-a
    ...
    apps_i_h = app_events_id.groupby(['timestamp_h']).is_installed.sum().reindex(hrange).fillna(0)
    apps_i_d = app_events_id.groupby(['timestamp_d']).is_installed.sum().reindex(drange).fillna(0)
    apps_a_h = app_events_id.groupby(['timestamp_h']).is_active.sum().reindex(hrange).fillna(0)
    apps_a_d = app_events_id.groupby(['timestamp_d']).is_active.sum().reindex(drange).fillna(0)
    ...
    newfigure('App usage per day for %s' % device_id, loc='ropright')
    plot_bar(norm(events_d),norm(apps_i_d),norm(apps_a_d), labels=['Events','Installed apps','Active apps'])
    plt.ylabel('Events / apps count')
    plt.show()
...
if __name__ == "__main__":
    ...
    a=pd.read_csv(dir_source+'./data_ori/events.csv', dtype=c, parse_dates=['timestamp'], date_parser=dateparse, index_col='event_id')
    
    dfe = unpack_date(a, 'timestamp')
    
    # Load app_events
    print('Reading app events...')
    dfa = pd.read_csv('./data_ori/app_events.csv', index_col='event_id')
